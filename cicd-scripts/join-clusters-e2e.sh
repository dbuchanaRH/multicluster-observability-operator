...
for ((i = 1; i <= CLUSTERPOOL_MANAGED_COUNT; i++)); do
  if $IS_HUB_TOKEN && [[ -n ${SHARED_DIR} ]] && [[ -f "${SHARED_DIR}/managed-${i}.json" ]]; then
    ...
    CTX_MANAGED_CLUSTER=$(kubectl config current-context)
...
for j in {1..60}; do
  for ((i = 1; i <= CLUSTERPOOL_MANAGED_COUNT; i++)); do
    if $IS_HUB_TOKEN && [[ -n ${CSR_CREATED[$i]} ]] && kubectl get csr --context "${CTX_HUB_CLUSTER}" 2>/dev/null | grep -q "managed-${i}"; then
      unset "CSR_CREATED[$i]"
